...
function EditProduct(props) {
    ...
    const findProduct = async () => {
        try {
            const response = await axios.get(`https://backend-vq0u.onrender.com/findProduct/${id}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.put(`https://backend-vq0u.onrender.com/editProduct/${id}`, { name, quantity, price });
